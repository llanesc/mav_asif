...
public:
    Mav1ASIF()
        ...
        timesync_sub_ =
                this->create_subscription<px4_msgs::msg::Timesync>(mav1_namespace_ + "fmu/timesync/out", 10,
...
//			}
            geometric_controller(mav1_des_, mav2_des_, control1_, control2_);
            RCLCPP_INFO(get_logger(), "\nroll:%.3f pitch:%.3f yaw:%.3f thrust:%.4f\n", control1_.roll, control1_.pitch,
                        control1_.yaw, control1_.thrust_body[2]);
...
void Mav1ASIF::geometric_controller(const Eigen::Vector4d &mav1_des,
                                    const Eigen::Vector4d &mav2_des,
                                    VehicleRatesSetpoint &control1,
                                    VehicleRatesSetpoint &control2) {
...
//	std::cout << theta1_cmd << std::endl;
    control1.thrust_body[2] = -compute_relative_thrust(
            -f_cmd_x1 * cos(eulers1.x()) * sin(eulers1.y()) + f_cmd_y1 * sin(eulers1.x())
            - f_cmd_z1 * cos(eulers1.x()) * cos(eulers1.y()), mav1_battery_status_.voltage_filtered_v,
            quad1_min_thrust_body_, quad1_max_thrust_body_);
    control2.thrust_body[2] = -compute_relative_thrust(
            -f_cmd_x2 * cos(eulers2.x()) * sin(eulers2.y()) + f_cmd_y2 * sin(eulers2.x())
            - f_cmd_z2 * cos(eulers2.x()) * cos(eulers2.y()), mav2_battery_status_.voltage_filtered_v,
            quad2_min_thrust_body_, quad2_max_thrust_body_);
 ...
 */
void Mav1ASIF::publish_offboard_control_mode() const {
 ...
 */
void Mav1ASIF::publish_vehicle_command(uint16_t command, float param1,
                                       float param2) const {
    VehicleCommand msg{};
    msg.timestamp = timestamp_.load();
    msg.param1 = param1;
